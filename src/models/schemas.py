...
class TrainingRequest(BaseModel):
    ...
    @classmethod
    def validate_target_column(cls, v, info):
        if info.data.get("problem_type"):
            if info.data["problem_type"] != ProblemType.CLUSTERING and not v:
                raise ValueError("target_column is required for supervised learning")
            if info.data["problem_type"] == ProblemType.CLUSTERING and v:
